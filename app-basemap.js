...
function inlineDataForTarget(target) {
  if (target === 'sites' && typeof window !== 'undefined' && window.__INLINE_SITE_DATA__) return window.__INLINE_SITE_DATA__;
  if ((target === 'boondockingZonesLocal' || target === 'boondockingZones') && typeof window !== 'undefined' && window.__INLINE_BOONDOCKING_ZONES__) return window.__INLINE_BOONDOCKING_ZONES__;
  return null;
}
...
async function loadFirstAvailable(urls, target = 'sites') {
  const attempts = [];
  const inline = inlineDataForTarget(target);
  if (inline && isFileProtocol()) {
    attempts.push({ url: 'inline-data', ok: true, status: 200, reason: 'Embedded data used for local file launch' });
  ...
  if (inline) {
    attempts.push({ url: 'inline-data', ok: true, status: 200, reason: 'Embedded fallback data used' });
    model.dataLoad[`${target}Attempted`] = [...attempts];
    model.dataLoad[`${target}Url`] = 'inline-data';
    model.dataLoad[`${target}Error`] = '';
    refreshStatusText();
